...
import { X, Upload, ArrowLeft, Folder, FileText } from 'lucide-react';
...
export const ProjectUploadModal: React.FC<ProjectUploadModalProps> = ({ isOpen, onClose }) => {
            ...
            <div className="flex items-center space-x-8">
              {[1, 2, 3].map((step) => (
                <div key={step} className="flex items-center space-x-3">
                  ...
                  >
                    {step === 1 ? 'Upload' : step === 2 ? 'Select Trade' : 'Upload Bid'}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div
                  ...
                  onDrop={handleDrop}
                  className="border-2 border-dashed border-slate-600 rounded-lg p-12 text-center bg-slate-700/50 hover:bg-slate-700/70 transition-colors cursor-pointer"
                  onClick={() => fileInputRef.current?.click()}
                >
                  <Upload size={48} className="mx-auto text-slate-400 mb-4" />
                  <p className="text-slate-300 mb-2">Drag and drop files</p>
                  <p className="text-slate-400 text-sm mb-4">.pdf, .dwg, .zip</p>
                ...
                {uploadedFiles.length > 0 && (
                  <div className="mt-4 space-y-2">
                    {uploadedFiles.map((file, index) => (
                      <div key={index} className="text-sm text-slate-300 bg-slate-700 px-3 py-2 rounded">
